function [h,t,t0,np]= pdp_svsim(Lam,lam,Gam,gam,num_cl,num_ch,b002,sdi,nlos)
%rng(0)
% S-V channel model
% Inputs
...
for k=1:num_ch % Loop over number of channels
    ...
    for i = 1:num_cl % Cluster loop %num_cl is 2 for dataset 1 and 2 and 3 for dataset 3
        Tr=0;
        num_rays = ceil(5*gam(i)*lam(i));
        pow_cluster = exp(-Tc/Gam);
        while (Tr < 10*gam(i))  % Ray loop
            t_val = Tc+Tr; % time of arrival of this ray
            bkl2 = b002*pow_cluster*gam(i)*exp(-Tr/gam(i)); % ray power, Eq.(2.14)
   ...
   %[h(1:np(k),k), t] = ex(h(1:np(k), k), t, np, num_ch, 1);
end
end